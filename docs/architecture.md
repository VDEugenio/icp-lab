...
- **Password**: compared against `DASHBOARD_PASSWORD_HASH` from the
  ...
  The variable accepts a comma-separated list of hashes; a login succeeds
  if the password matches any of them (all grant identical access — there
  are no per-password permissions or session distinctions).
